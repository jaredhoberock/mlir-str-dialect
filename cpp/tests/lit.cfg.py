...
str_plugin = plugin('STR_DIALECT_PLUGIN', os.path.join(os.path.dirname(__file__), '..', 'build', 'libstr_dialect.so'))
config.substitutions.append(('mlir-opt', f'{tool('mlir-opt')} --load-dialect-plugin={str_plugin}'))
